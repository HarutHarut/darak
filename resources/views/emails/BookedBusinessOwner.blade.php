...
<div style="max-width: 600px; width: 100%; padding: 30px;">
    ...
    <h2 style="color: #ff0000; margin-top: 15px;">{{ __('general.emails.haveANewBooking') }}.</h2>
    ...
    <table style="width: 100%;">
        <thead>
        <tr>
            <td>{{ __('general.emails.clientName') }}.</td>
            <td>{{ $user->name }}</td>
        </tr>
        <tr>
            <td>{{ __('general.emails.clientEmail') }}.</td>
            <td>{{ $user->email }}</td>
        </tr>
        <tr>
            <td>{{ __('general.emails.clientTel') }}.</td>
            <td>{{ $user->phone }}</td>
        </tr>
    ...
    <h2 style="color: #0070c0; margin-top: 80px; font-style: italic;">
        <span class="red-text">{{ __('general.emails.goodLuck') }},</span><br>
        {{ __('general.emails.sincerely') }},<br>
        {{ __('general.emails.lugLockersTeam') }}
    </h2>
    <p style="font-weight: 600; font-size: 12px;">{{ __('general.emails.cancelBookingAfterOne') }}.</p>
    @if($order->status !== 'canceled')
        <a href="{{ config('app.url') . 'cancel-booking?booking_number=' . base64_encode($order->booking_number) }}" class="cancel-booking-btn">
            {{ __('general.emails.cancelBooking') }}
        </a>
    @endif
</div>
</body>
